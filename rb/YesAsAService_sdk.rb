...
require_relative 'YesAsAService_types'
...
class YesAsAServiceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YesAsAServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YesAsAServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YesAsAServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YesAsAServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YesAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YesAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YesAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = YesAsAServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YesAsAServiceError.new(
        "graphql_error", "YesAsAServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = YesAsAServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
