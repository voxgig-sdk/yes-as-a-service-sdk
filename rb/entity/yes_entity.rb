...
class YesEntity
  ...
  def make
    opts = @_entopts.dup
    YesEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YesAsAServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Yes, Hash] the current Yes data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YesAsAServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Yes fields)
  def match_get
  ...
  # Load a single Yes.
  #
  # @param reqmatch [YesLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Yes.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Yes, Hash] the loaded Yes; raises YesAsAServiceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YesAsAServiceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
