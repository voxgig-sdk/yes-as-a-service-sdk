...
describe("YesEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set YES_AS_A_SERVICE_TEST_YES_ENTID JSON to run live")
      return
    end
...
function yes_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("YES_AS_A_SERVICE_TEST_YES_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["YES_AS_A_SERVICE_TEST_YES_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["YES_AS_A_SERVICE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["YES_AS_A_SERVICE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
    idmap = idmap_resolved,
    env = env,
    explain = env["YES_AS_A_SERVICE_TEST_EXPLAIN"] == "TRUE",
    live = live,
