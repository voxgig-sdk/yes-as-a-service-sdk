...
local YesEntity = {}
YesEntity.__index = YesEntity


function YesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, YesEntity)
  self._name = "yes"
  self._client = client
...
function YesEntity:get_name()
  return self._name
end


function YesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return YesEntity.new(self._client, opts)
end
...
function YesEntity:mark_deleted()
  self._deleted = true
end


function YesEntity:deleted()
  return true == self._deleted
end


function YesEntity:data_set(args)
  if args ~= nil then
...
function YesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function YesEntity:match_set(args)
  if args ~= nil then
...
function YesEntity:stream(action, args, callopts)
...
---@param reqmatch YesLoadMatch
---@param ctrl? table
---@return Yes
---@return string? err
function YesEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function YesEntity:_run_op(ctx, post_done)
  local utility = self._utility
