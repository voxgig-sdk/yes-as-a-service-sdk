...
require("yes-as-a-service_types")
...
local YesAsAServiceSDK = {}
YesAsAServiceSDK.__index = YesAsAServiceSDK
...
YesAsAServiceSDK._make_feature = _make_feature


function YesAsAServiceSDK.new(options)
  local self = setmetatable({}, YesAsAServiceSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function YesAsAServiceSDK:options_map()
...
function YesAsAServiceSDK:get_utility()
  return Utility.copy(self._utility)
end


function YesAsAServiceSDK:get_root_ctx()
  return self._rootctx
end


function YesAsAServiceSDK:prepare(fetchargs)
...
function YesAsAServiceSDK:direct(fetchargs)
...
-- by passing it.
function YesAsAServiceSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function YesAsAServiceSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "YesAsAServiceSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function YesAsAServiceSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = YesAsAServiceSDK.new(sdkopts)
  sdk.mode = "test"
