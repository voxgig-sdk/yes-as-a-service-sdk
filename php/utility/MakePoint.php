...
// YesAsAService SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class YesAsAServiceMakePoint
{
    public static function call(YesAsAServiceContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof YesAsAServiceError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = YesAsAServiceHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = YesAsAServiceHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
