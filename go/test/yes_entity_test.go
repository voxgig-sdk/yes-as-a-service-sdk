...
func TestYesEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		yesRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.yes")))
...
func yesBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YES_AS_A_SERVICE_TEST_YES_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YES_AS_A_SERVICE_TEST_YES_ENTID"])
