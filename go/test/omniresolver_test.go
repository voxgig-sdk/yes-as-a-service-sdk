...
import (
	...
	omni "github.com/voxgig-sdk/yes-as-a-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YesAsAServiceError); is {
				out := map[string]any{
					"name":    "YesAsAServiceError",
...
func omniCtx(arg any, client *sdk.YesAsAServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YesAsAServiceSDK); is {
			client = live
