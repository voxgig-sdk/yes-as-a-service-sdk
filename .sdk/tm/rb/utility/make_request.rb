...
module YesAsAServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YesAsAServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = YesAsAServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YesAsAServiceResponse.new(fetched)
    else
