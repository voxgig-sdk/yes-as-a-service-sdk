...
// YesAsAService SDK feature test
//
...
class FtCtrl extends YesAsAServiceControl
{
...
class FtClient
{
    ...
    private YesAsAServiceUtility $utility;

    public function __construct(YesAsAServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): YesAsAServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YesAsAServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YesAsAServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YesAsAServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YesAsAServiceConfig::make_feature($name);
    ...
    private static function buildUrl(YesAsAServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YesAsAServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YesAsAServiceResponse($fetched);
        }
    ...
    private function fail(YesAsAServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YesAsAServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
