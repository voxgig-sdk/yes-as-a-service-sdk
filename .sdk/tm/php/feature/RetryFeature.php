...
// YesAsAService SDK retry feature
...
class YesAsAServiceRetryFeature extends YesAsAServiceBaseFeature
{
    ...
    public function init(YesAsAServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YesAsAServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YesAsAServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YesAsAServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
