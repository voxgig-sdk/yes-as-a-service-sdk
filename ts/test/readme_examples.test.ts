...
//     (\`new YesAsAServiceSDK(...)\` / \`YesAsAServiceSDK.test(...)\` become
//     \`YesAsAServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YesAsAServiceSDK'
...
const TEST_SEED = {"entity":{"yes":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YesAsAServiceSDK(...)\` and any
// \`YesAsAServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YesAsAServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YesAsAServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
