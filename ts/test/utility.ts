...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.YESASASERVICE_TEST_LIVE ||
    'TRUE' === process.env.YESASASERVICE_TEST_OVERRIDE
  ) {
  ...
  m.YESASASERVICE_TEST_EXPLAIN = process.env.YESASASERVICE_TEST_EXPLAIN || m.YESASASERVICE_TEST_EXPLAIN
