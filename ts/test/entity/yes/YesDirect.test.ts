...
describe('YesDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new YesAsAServiceSDK({
  ...
  test('direct-load-yes', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-yes', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['YES_AS_A_SERVICE_TEST_YES_ENTID']
