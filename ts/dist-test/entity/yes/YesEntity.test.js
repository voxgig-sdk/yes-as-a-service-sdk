...
(0, node_test_1.describe)('YesEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when YES_AS_A_SERVICE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('YES_AS_A_SERVICE_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.YesAsAServiceSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'yes.' + op, live))
                return;
        }
        ...
        let yes_ref01_data = Object.values(setup.data.existing.yes)[0];
        // LOAD
        const yes_ref01_ent = client.Yes();
        const yes_ref01_match_dt0 = {};
        const yes_ref01_data_dt0 = (await yes_ref01_ent.load(yes_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != yes_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/yes/YesTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.YesAsAServiceSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['yes01', 'yes02', 'yes03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.YesAsAServiceSDK(merge([
...
//# sourceMappingURL=YesEntity.test.js.map