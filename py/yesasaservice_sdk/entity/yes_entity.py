...
from yesasaservice_sdk.yesasaservice_types import (
    Yes,
    YesLoadMatch,
)
...
class YesEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return YesEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Yes:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Yes().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
