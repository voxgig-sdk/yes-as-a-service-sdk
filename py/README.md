...
The Python SDK for the YesAsAService API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Yes(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.YesAsAService(None).load(
    {"id": "test01"}, None
)
