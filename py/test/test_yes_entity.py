    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set YES_AS_A_SERVICE_TEST_YES_ENTID JSON to run live")
        client = setup["client"]
...
def _yes_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("YES_AS_A_SERVICE_TEST_YES_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("YES_AS_A_SERVICE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("YES_AS_A_SERVICE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("YES_AS_A_SERVICE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
