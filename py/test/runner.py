...
class YesAsAServiceTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YesAsAServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YesAsAServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YesAsAServiceTestRunner.getenv("YES_AS_A_SERVICE_TEST_LIVE")
        override = YesAsAServiceTestRunner.getenv("YES_AS_A_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YesAsAServiceTestRunner.getenv(key)
        ...
        explain = YesAsAServiceTestRunner.getenv("YES_AS_A_SERVICE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if YesAsAServiceTestRunner._test_control is not None:
            return YesAsAServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YesAsAServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YesAsAServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YesAsAServiceTestRunner.load_test_control()
...
def load_env_local():
    YesAsAServiceTestRunner.load_env_local()


def env_override(m):
    return YesAsAServiceTestRunner.env_override(m)


def entity_data(v):
    return YesAsAServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YesAsAServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YesAsAServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YesAsAServiceTestRunner.load_test_control()


def live_client_options():
    return YesAsAServiceTestRunner.live_client_options()


def live_delay_ms():
    return YesAsAServiceTestRunner.live_delay_ms()
