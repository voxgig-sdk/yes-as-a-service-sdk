# yes-as-a-service-mcp

[MCP](https://modelcontextprotocol.io) server exposing the YesAsAService SDK as
two agent tools — `yes-as-a-service_list` and `yes-as-a-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yes-as-a-service-mcp)
make build
...
claude mcp add --scope user yes-as-a-service \
  -- /absolute/path/to/yes-as-a-service-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./yes-as-a-service-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// yes-as-a-service_list: first page of records
{ "entity": "yes" }
{ "entity": "yes", "query": { } }

// yes-as-a-service_load: one record by id
{ "entity": "yes", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yes-as-a-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yes-as-a-service \
     -- "$PWD"/dist/*/yes-as-a-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yes-as-a-service_list` and `yes-as-a-service_load` tools now appear
   in new sessions. Ask the agent to *"list yes using yes-as-a-service"*
   and it calls `yes-as-a-service_list` with `{"entity":"yes"}`.
...
### Call the `yes-as-a-service_list` tool
...
### Call the `yes-as-a-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `yes-as-a-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yes-as-a-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./yes-as-a-service-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"yes-as-a-service_load","arguments":{"entity":"yes","query":{"id":1}}}}'
```
